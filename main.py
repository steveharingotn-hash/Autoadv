...
class AdSlot:
    def __init__(self, slot_id, token, channels, delay, message):
        ...
        self.delay = max(10, int(delay))  # Minimum 10 seconds
    ...
    async def start(self):
        ...
        async def on_ready():
            ...
            print(f"[Slot {self.slot_id}] 🚀 Starting Auto Adv...")
            self.task = asyncio.create_task(self.advertise())

        try:
            await self.client.start(self.token)
        except Exception as e:
            print(f"[Slot {self.slot_id}] ❌ LOGIN FAILED: {str(e)}")
            ...
            for cid in self.channels:
                try:
                    ...
                    if channel:
                        ...
                        print(f"[Slot {self.slot_id}] ✅ Message sent to {cid}")
                    else:
                        print(f"[Slot {self.slot_id}] ❌ Channel {cid} not found")
                except Exception as e:
                    print(f"[Slot {self.slot_id}] Send Error: {str(e)}")
...
class SetupModal(discord.ui.Modal, title="Setup Slot 1"):
    def __init__(self):
        ...
        self.channels = discord.ui.TextInput(label="Target Channels *", placeholder="1471484118930952399", default=", ".join(data.get("channels", [])), required=True)
