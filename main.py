...
class AdSlot:
    ...
    async def start(self):
        print(f"[Slot {self.slot_id}] Attempting login...")
        ...
        async def on_ready():
            ...
            print(f"[Slot {self.slot_id}] 🚀 Advertising started | Delay: {self.delay}s")
            self.task = asyncio.create_task(self.advertise())

        try:
            await self.client.start(self.token, bot=False)  # Self-bot
        except Exception as e:
            print(f"[Slot {self.slot_id}] ❌ LOGIN FAILED: {e}")

    async def advertise(self):
        print(f"[Slot {self.slot_id}] Advertising loop running...")
        while True:
            for cid in self.channels:
                try:
                    ...
                    print(f"[Slot {self.slot_id}] Error sending: {e}")
...
class ReplicaControlPanel(discord.ui.View):
    ...
    async def slot1(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(f"**Slot 1 Status:** {status}", ephemeral=True)
    ...
    async def start(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("🚀 Starting Slot 1... Check Railway Logs!", ephemeral=True)
